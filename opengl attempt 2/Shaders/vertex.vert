...
layout(std140, binding=3) buffer One
{
  vec4 Coordsin[];
};

layout(std140, binding=4) buffer Two
{
  vec4 Dimentions[];
};
...
void main()
{
	...
	scale2 = Dimentions[gl_InstanceID].xyz;


	CurrentPosition = vec3(CamPos * vec4(positions*scale2, 1.0));
